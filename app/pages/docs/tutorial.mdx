...
In questo tutorial, vi guideremo attraverso la creazione di una semplice
applicazione di votazione.

Supponiamo che abbiate già [installato Blitz] (./ get-started). Potete
sapere se Blitz è installato e quale versione hai eseguendo il seguente
...
Se Blitz è installato, dovreste vedere la versione della vostra
installazione. Se non lo è, avrete un errore sul tipo di “command not
found: blitz”.

## Creare una nuova applicazione {#creating-a-new-app}

Dalla riga di comando, `cd` all'interno della directory in cui vorreste
creare la vostra applicazione e quindi lanciate il seguente comando:
