...
- La directory `app/` contiene la maggior parte del progetto. Qui è dove
  verrà inserita ogni pagina o rotta per le API.
...
già non lo sei, e lancia il seguente comando:
...
Ora che il server è attivo, apri il link
...
`Choice`. Una `Question` conterrà il testo della domanda e un elenco di
scelte. Una `Choice` conterrà il testo della scelta, un conteggio dei voti
e una domanda associata. Blitz genererà automaticamente un ID, un
...
ancora aggiunto il campo `Choice` al model `Question`. Quindi scegliamo `false` 
quando ci viene chiesto di eseguire la migration:
...
Adesso il nostro database è pronto ed è stato anche generato un client Prisma. Proseguiamo col giocare col client Prisma!
...
## Aggiornare il codice generato per gli attributi del model {#update-generated-code-for-our-model-attributes}
...
nel frattempo andiamo a corregere le pagine generate.

### Pagine per le Question {#question-pages}
...
### Mutation `createQuestion` {#create-question-mutation}
...
### Mutation `deleteQuestion` {#delete-question-mutation}
...
## Aggiungere delle scelte al form della domanda {#adding-choices-to-the-question-form}
...
## Elencare le choice {#listing-choices}
...
## Lasciamo che le persone votino sulle domande! {#let-people-vote-on-questions}
...
## Infine, consentiamo di modificare le scelte per una domanda esistente {#edit-choices-for-question}
...
Apri il file `app/questions/mutations/updateQuestion.ts` e effettua le
seguenti modifiche:
